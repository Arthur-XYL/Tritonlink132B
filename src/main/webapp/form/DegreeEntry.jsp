...
<table border="1">
    ...
    <tr>
        ...
        <th>Technical Elective Required Units</th>
        <th>Actions</th>
    </tr>
    </thead>
    <tbody>
    <tr>
        <form method="post">
            <td>N/A</td>
            <td><select name="type" required><option value="Bachelor">Bachelor</option><option value="Master">Master</option><option value="PhD">PhD</option></select></td>
            ...
            <td><input type="number" name="technical_elective_required_units" required></td>
    ...
    <%
        ...
        try {
            ...
            if ("insert".equals(action)) {
                String insertSQL = "INSERT INTO degree (type, name, required_units, minimum_grade, department_name, upper_division_required_units, technical_elective_required_units) VALUES (CAST(? AS degree_type), ?, ?, CAST(? AS numeric), ?, ?, ?)";
                ...
                pstmt.setBigDecimal(4, new java.math.BigDecimal(request.getParameter("minimum_grade"))); // Correct casting
                ...
                pstmt.setInt(7, Integer.parseInt(request.getParameter("technical_elective_required_units")));
                pstmt.executeUpdate();
            }

            if ("update".equals(action)) {
                String updateSQL = "UPDATE degree SET type = CAST(? AS degree_type), name = ?, required_units = ?, minimum_grade = CAST(? AS numeric), department_name = ?, upper_division_required_units = ?, technical_elective_required_units = ? WHERE degree_id = ?";
            ...
            String selectSQL = "SELECT degree_id, type, name, required_units, minimum_grade, department_name, upper_division_required_units, technical_elective_required_units FROM degree ORDER BY degree_id";
            ...
            while (rs.next()) {
                ...
                int technicalElectiveRequiredUnits = rs.getInt("technical_elective_required_units");
    %>
    <tr>
        <form method="post">
            <td><%= degreeId %></td>
            <td><select name="type" required><option value="Bachelor" <%= "Bachelor".equals(type) ? "selected" : "" %>>Bachelor</option><option value="Master" <%= "Master".equals(type) ? "selected" : "" %>>Master</option><option value="PhD" <%= "PhD".equals(type) ? "selected" : "" %>>PhD</option></select></td>
            ...
            <td><input type="number" name="technical_elective_required_units" value="<%= technicalElectiveRequiredUnits %>" required></td>
            <td>
