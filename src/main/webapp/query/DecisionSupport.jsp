...
<html>
<head>
    <title>Comprehensive Grade Reports</title>
</head>
<body>
<h1>Comprehensive Grade Report Generator</h1>
<form method="post">
    <label for="reportType">Report Type:</label>
    <select name="reportType" id="reportType">
        <option value="quarterGrades">Grade Distribution for Specific Course and Quarter</option>
        <option value="professorGradesYears">Grade Distribution by Professor Over the Years</option>
        <option value="courseGradesYears">Grade Distribution for a Course Over the Years</option>
        <option value="averageGrade">Average Grade Given by a Professor Over the Years</option>
    </select><br>

    <label for="courseId">Course ID:</label>
    <input type="number" id="courseId" name="courseId" required><br>

    <label for="professorName">Professor Name (if applicable):</label>
    <input type="text" id="professorName" name="professorName"><br>

    <label for="quarter">Select Quarter (if applicable):</label>
    <select id="quarter" name="quarter">
...
<%
    ...
    if (courseIdParam != null && !courseIdParam.isEmpty()) {
        ...
        try {
            ...
            if ("quarterGrades".equals(reportType) && quarter != null && !quarter.isEmpty()) {
                query = "SELECT CASE WHEN gc.letter_grade IN ('A+', 'A', 'A-') THEN 'A' " +
                        "WHEN gc.letter_grade IN ('B+', 'B', 'B-') THEN 'B' " +
                        "WHEN gc.letter_grade IN ('C+', 'C', 'C-') THEN 'C' " +
                        "WHEN gc.letter_grade IN ('D+', 'D', 'D-') THEN 'D' " +
                        "ELSE 'Other' END AS grade_category, COUNT(*) AS count " +
                        "FROM enrollment e JOIN section sec ON e.section_id = sec.section_id AND e.class_id = sec.class_id " +
                        "JOIN class cl ON sec.class_id = cl.class_id JOIN faculty fac ON sec.faculty_name = fac.faculty_name " +
                        "JOIN grade_conversion gc ON e.grade::text = gc.letter_grade " +
                        "WHERE cl.course_id = ? AND fac.faculty_name = ? AND cl.quarter::text = ? " +
                        "AND e.grade_type = 'Letter Grade' AND e.grade NOT IN ('N/A') GROUP BY grade_category";
                pstmt = conn.prepareStatement(query);
                pstmt.setInt(1, courseId);
                pstmt.setString(2, professorName);
                pstmt.setString(3, quarter);
            } else if ("professorGradesYears".equals(reportType)) {
                query = "SELECT CASE WHEN gc.letter_grade IN ('A+', 'A', 'A-') THEN 'A' " +
                        "WHEN gc.letter_grade IN ('B+', 'B', 'B-') THEN 'B' " +
                        "WHEN gc.letter_grade IN ('C+', 'C', 'C-') THEN 'C' " +
                        "WHEN gc.letter_grade IN ('D+', 'D', 'D-') THEN 'D' " +
                        ...
                        "JOIN class cl ON sec.class_id = cl.class_id JOIN faculty fac ON sec.faculty_name = fac.faculty_name " +
                        "JOIN grade_conversion gc ON e.grade::text = gc.letter_grade " +
                        "WHERE cl.course_id = ? AND fac.faculty_name = ? AND e.grade_type = 'Letter Grade' AND e.grade NOT IN ('N/A') " +
                        "GROUP BY grade_category";
                pstmt = conn.prepareStatement(query);
                pstmt.setInt(1, courseId);
                pstmt.setString(2, professorName);
            } else if ("courseGradesYears".equals(reportType)) {
                query = "SELECT CASE WHEN gc.letter_grade IN ('A+', 'A', 'A-') THEN 'A' " +
                        "WHEN gc.letter_grade IN ('B+', 'B', 'B-') THEN 'B' " +
                        "WHEN gc.letter_grade IN ('C+', 'C', 'C-') THEN 'C' " +
                        "WHEN gc.letter_grade IN ('D+', 'D', 'D-') THEN 'D' " +
                        "ELSE 'Other' END AS grade_category, COUNT(*) AS count " +
                        "FROM enrollment e JOIN class cl ON e.class_id = cl.class_id " +
                        "JOIN grade_conversion gc ON e.grade::text = gc.letter_grade " +
                        "WHERE cl.course_id = ? AND e.grade_type = 'Letter Grade' AND e.grade NOT IN ('N/A') " +
                        "GROUP BY grade_category";
                pstmt = conn.prepareStatement(query);
                pstmt.setInt(1, courseId);
            } else if ("averageGrade".equals(reportType)) {
                query = "SELECT AVG(gc.number_grade) AS average_grade " +
                        "FROM enrollment e JOIN section sec ON e.section_id = sec.section_id AND e.class_id = sec.class_id " +
                        "JOIN class cl ON sec.class_id = cl.class_id JOIN faculty fac ON sec.faculty_name = fac.faculty_name " +
                        "JOIN grade_conversion gc ON e.grade::text = gc.letter_grade " +
                        "WHERE cl.course_id = ? AND fac.faculty_name = ? AND e.grade_type = 'Letter Grade' AND e.grade NOT IN ('N/A') ";
            ...
            if ("averageGrade".equals(reportType)) {
                if (rs.next()) {
                    double averageGrade = rs.getDouble("average_grade");
                    out.println("<p>Average Grade: " + averageGrade + "</p>");
                } else {
                    out.println("<p>No grade data found for the specified criteria.</p>");
                }
            } else {
